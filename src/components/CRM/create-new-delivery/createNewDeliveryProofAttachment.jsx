...
export default function CreateNewDeliveryProofAttachment({ inputDisable }) {
  ...
  return (
    <div className="createNewDelivery-attachment-container">
      <div className="createNewDelivery-upload-container">
        <input
          type="file"
          ref={fileInputRef}
          hidden
        ...
        />
        <div
          className="createNewDelivery-upload-btn"
      ...
      {/* Uploaded files */}
      <nav className="createNewDelivery-attached-files">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="18"
          height="18"
      ...
      {uploadedFiles.length > 0 || backendFiles.length > 0 ? (
        <>
          {uploadedFiles.length > 0 && (
            <>
              {uploadedFiles.map((file, index) => (
                <div
                  key={`upload-${index}`}
                  className="createNewDelivery-file-item"
                >
                  <nav>{file.name}</nav>
                  <div className="createNewDelivery-file-actions">
                    <a
        ...
        <p className="createNewDelivery-no-data-found">No file uploaded yet.</p>
      )}
    </div>
  );
}

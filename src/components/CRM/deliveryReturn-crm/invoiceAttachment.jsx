...
export default function invoiceAttachment({ inputDisable }) {
  ...
  return (
    <div className="createNewDeliveryReturn-attachment-container">
      <div className="createNewDeliveryReturn-upload-container">
        <input
          type="file"
          ref={fileInputRef}
          hidden
      ...
      {uploadedFiles.length > 0 && (
        <>
          <nav className="createNewDeliveryReturn-attached-files">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="18"
              height="18"
          ...
          {uploadedFiles.map((file, index) => (
            <div key={`upload-${index}`} className="createNewDeliveryReturn-file-item">
              <nav>{file.name}</nav>
      ...
      {backendFiles.length > 0 && (
        <>
          ...
          {backendFiles.map((file, index) => (
            <div
              key={`backend-${index}`}
              className="createNewDeliveryReturn-file-item"
            >
              <nav>{file.name}</nav>
              <div className="createNewDeliveryReturn-file-actions">
                <a

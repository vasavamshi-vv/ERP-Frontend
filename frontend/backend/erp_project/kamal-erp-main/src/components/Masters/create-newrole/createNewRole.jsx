    ...
    try {
      ...
      do {
        const response = await axios.get(
          `http://13.239.191.236:8000/api/departments/?page=${page}`,
          {
            headers: {
              Authorization: `Token ${token}`,
              "Content-Type": "application/json",
      ...
      const roleResponse = await axios.get(
        "http://13.239.191.236:8000/api/roles/",
        {
          headers: {
            Authorization: `Token ${token}`,
            "Content-Type": "application/json",
...
// };
  ...
  try {
    ...
    if (editRoleOnly && editRole?.id) {
      // ✅ Edit mode → use PUT
      res = await axios.put(`http://13.239.191.236:8000/api/roles/${editRole.id}/`, data, config);
      toast.success("Role updated successfully!");
    } else {
      // ✅ Create mode → use POST
      res = await axios.post("http://13.239.191.236:8000/api/roles/", data, config);

...
export default function createDepartmentRole({
  ...
  useEffect(() => {
    const fetchBranches = async () => {
      try {
        const response = await axios.get("http://13.239.191.236:8000/api/branches/", {
          headers: {
  ...
  const fetchRoles = async (departmentId) => {
    ...
    try {
      const response = await axios.get(
        `http://13.239.191.236:8000/api/roles/?department=${departmentId}`,
        {
          headers: {
            Authorization: `Token ${token}`,
            "Content-Type": "application/json",
  ...
  const fetchExampleRoles = async () => {
    setRolesLoading(true);
    try {
      const response = await axios.get("http://13.239.191.236:8000/api/roles/?page=1&per_page=3", {
        headers: {
          Authorization: `Token ${token}`,
          "Content-Type": "application/json",
        },
  ...
  const deleteRole = async (roleId) => {
    ...
    if (okDel) {
      try {
        await axios.delete(`http://13.239.191.236:8000/api/roles/${roleId}/`, {
          headers: {
            Authorization: `Token ${token}`,
            "Content-Type": "application/json",
          },
  ...
  async function handleNewDepartmentSubmit(e) {
    ...
    try {
      ...
      const url = "http://13.239.191.236:8000/api/departments/";

  ...
  const fetchData = async () => {
    try {
      const [branchRes, deptRes, roleRes, userRes] = await Promise.all([
        axios.get("http://13.239.191.236:8000/api/branches/", {
          headers: { Authorization: `Token ${token}` },
        }),
        axios.get("http://13.239.191.236:8000/api/departments/", {
          headers: { Authorization: `Token ${token}` },
        }),
        axios.get(`http://13.239.191.236:8000/api/roles/${createUserForm.department ? `?department=${createUserForm.department}` : ''}`, {
          headers: { Authorization: `Token ${token}` },
        }),
        axios.get("http://13.239.191.236:8000/api/users/", {
          headers: { Authorization: `Token ${token}` },
        }),
      ]);
    ...
    try {
      ...
      if (editCreateUser && edituser.id) {
        response = await axios.put(
          `http://13.239.191.236:8000/api/users/${edituser.id}/`,
          formData,
          config
        );
        ...
        response = await axios.post("http://13.239.191.236:8000/api/users/", formData, config);
